...
class Patient(BaseModel):
    ...
    name: Annotated[
        str, Field(..., description="Name of the Patient", examples=["sam"])
    ]
...
def sort_patients(
    ...
    if sort_by not in valid_feilds:
        raise HTTPException(
            status_code=400, detail=f"Invalid feild select from {valid_feilds}"
        )

    if order not in ["asc", "desc"]:
        raise HTTPException(
            status_code=400, detail="Invalid order select from asc or desc"
        )
